...
function validateValue(
  ...
  if (param.enum && !param.enum.includes(value)) {
    throw new Error(
      `Invalid value at '${path}': expected one of [${param.enum.join(
        ", "
      )}], got '${value}'`
    );
  }
...
export function validateArguments(
  ...
  try {
    ...
    throw new Error(
      `Validation failed for function '${descriptor.name}': ${
...
export function prettyPrintMessages(messages: any[]): void {
