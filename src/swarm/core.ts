...
export class Swarm {
  ...
  private async getChatCompletion(
    ...
    const tools = agent.functions.map((func) =>
      functionToJson(func.descriptor)
    );
    // Hide context_variables from model
    tools.forEach((tool) => {
      delete tool.function.parameters.properties[CTX_VARS_NAME];
      const requiredIndex =
        tool.function.parameters.required.indexOf(CTX_VARS_NAME);
      if (requiredIndex !== -1) {
  ...
  async run(options: SwarmRunOptions): Promise<Response | AsyncIterable<any>> {
    const {
      agent,
      messages,
      context_variables = {},
